...
- (void)doConvertFile:(NSString*)url srcFilePos:(SInt64)pos {
    ...
    try {
        ...
        if (self.rtmpFlag) {
            [self.graph setOutputVolume:0.0];
        }
        ...
        while (!self.ab->stopRunloop) {
            ...
            if (error == noErr && ioOutputDataPackets > 0 && fillBufList.mBuffers[0].mDataByteSize > 0) {
                ...
                if (self.delegate != nil && [self.delegate respondsToSelector:@selector(AQConverter:publishAudioBuf:numberBytes:)]) {
                    [self.delegate AQConverter:self publishAudioBuf:fillBufList.mBuffers[0].mData numberBytes:fillBufList.mBuffers[0].mDataByteSize];
                }
            }
        }
